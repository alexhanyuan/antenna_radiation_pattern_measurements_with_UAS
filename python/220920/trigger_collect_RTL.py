...
# RTL stream takes about 4 seconds to start

buttonPin = 16   # trigger input from pixhawk, GPIO 23
...
def loop():
    while True:
        if GPIO.input(buttonPin) == GPIO.LOW: # if triggered
            returnCode = subprocess.call("python3 cli_collect.py --frequency 121500000 --RF_gain 16 --num_buffers 280", shell=True)  # run collect
